...
const LocalizedRoutes: React.FunctionComponent<IProps> = (props) => {
	const { applySwitch = false, routes = [], localize: localizeRouteConfig = true } = props;

	const getConfig = (config: Route[]) =>
		config.map((route: ILocalizedRouteProps | IRedirectProps | any, index: number) => {
			const {
				children,
				component,
				exact,
						...
						<Translator
							render={(language: string) => (
								<LocalizedRedirect
									...
									localize={
										localize === false ? false : localize || localizeRouteConfig
									}
									{...{ to, push, path, exact, strict }}
								/>
							)}
						/>
					</React.Fragment>
